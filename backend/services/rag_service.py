...
from models.material import Material, MaterialChunk, MaterialVisualAsset
...
class RAGService:
    ...
    def index_material(self, material, chunks):
        ids = []
        documents = []
        embeddings = []
        metadatas = []
        ...
        for index, content in enumerate(chunks):
            chroma_id = f"user-{material.user_id}-material-{material.id}-chunk-{index}"
    ...
    def reindex_material(self, material, chunks):
        self.delete_material_vectors(material.user_id, material.id)
        MaterialChunk.query.filter_by(material_id=material.id).delete()
        db.session.flush()
        self.index_material(material, chunks)

    def index_visual_assets(self, material, asset_payloads, reindex=False):
        if reindex:
        ...
        for payload in asset_payloads:
            ...
            chroma_id = f"user-{material.user_id}-material-{material.id}-visual-{index}"
    ...
    def answer(self, user_id, question, material_id=None, folder_id=None, conversation=None):
        if material_id:
            material = Material.query.filter_by(id=material_id, user_id=user_id).first()
            if not material:
                raise ValueError("资料不存在")
            if material.status != "ready":
                raise ValueError("资料尚未处理完成")
        elif folder_id:
            ready_count = Material.query.filter_by(
                user_id=user_id,
                folder_id=folder_id,
                status="ready",
            ).count()
            if ready_count == 0:
                raise ValueError("该文件夹下暂无可问答的资料")

        text_references = self.search(user_id, question, material_id, folder_id)
        visual_references = self.search_visual(user_id, question, material_id, folder_id)
